...
<span class=\"ank_n\">Баллы:</span> <span class=\"ank_d\">$user[balls]</span><br />\n";
    ...
    if ($user['level']>0) {
        if ($user['ip']!=0) {
            echo "<span class=\"ank_n\">IP:</span> <span class=\"ank_d\">".long2ip($user['ip'])."</span><br />\n";
        }
        if ($user['ua']!=null) {
            echo "<span class=\"ank_n\">UA:</span> <span class=\"ank_d\">$user[ua]</span><br />\n";
        }
        if (opsos($user['ip'])) {
            echo "<span class=\"ank_n\">Пров:</span> <span class=\"ank_d\">".opsos($user['ip'])."</span></div><br />";
        }
    }
} else {
    echo "
