...
/* Бан пользователя */ 
if ($db->query("SELECT COUNT(*) FROM `ban` WHERE `razdel` = 'files' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0' OR `navsegda` = '1')")!=0)
{
...
$k_post=$db->query("SELECT COUNT(*) FROM `obmennik_files` WHERE `opis` like '%".my_esc($q_search)."%' OR `name` like '%".my_esc($q_search)."%'");
$k_page=k_page($k_post,$set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];
if ($k_post==0)echo "<div class=\"p_t\">\nНет результатов</div>\n";
$q=$db->query("SELECT * FROM `obmennik_files` WHERE `opis` like '%".my_esc($q_search)."%' OR `name` like '%".my_esc($q_search)."%' ORDER BY `time` DESC LIMIT $start, $set[p_str]");
$i=0;
while ($post = $q->row())
{$k_p=$db->query("SELECT COUNT(*) FROM `obmennik_komm` WHERE `id_file` = '$post[id]'");
$ras=$post['ras'];
