...
SELECT COUNT(`read`) FROM `notification` WHERE `id_user` = ?i AND `read` = "0") k_notif',
            ...
            if ($cnt['k_new'] != 0 && $cnt['k_new_fav'] == 0) {
                ?><a href="/new_mess.php"><center><img src="/style/icons/icon_pochta22.gif" alt="DS" /><span class="off">(<?=$cnt['k_new']?>)</span></center></a><?php
            } else {
            ...
            if ($cnt['k_frend'] > 0) {
                ?>
				<td class="auts">
				<a href="/user/frends/new.php"><center><img src="/style/icons/icon_druzya.gif" alt="DS" /><span class="off">(<?=$cnt['k_frend']?>)</span></center></a>
				</td>
				<?php
            }
