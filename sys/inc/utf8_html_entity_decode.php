...
function utf8_html_entity_decode($s, $is_htmlspecialchars = false)
{
    ...
    if (($pos = strpos($s, '&#')) !== false) {  #speed optimization
        ...
        $s = preg_replace_callback(
            '/&#((x)[\da-fA-F]{2,4}|\d{1,4});/s', function( $m ) use ($htmlspecialchars_flip) {
                return (array_key_exists($a = pack('C', $d = isset($m[2]) ? hexdec($m[1]) : $m[1]), $htmlspecialchars_flip) && ! $is_htmlspecialchars) ?
             $htmlspecialchars_flip[$a] : iconv('UCS-2BE', 'UTF-8', pack('n', $d)); }, $s, -1, $pos);
    }
