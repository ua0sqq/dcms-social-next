	...
	*/
	public static function avatar($user = 0, $type = 0)
	{
		...
		*/
		...
		$AVATAR = null;
		$icon = null;
		if ($user != 0) $ank =go\DB\query('SELECT `pol`, `id`, `group_access` FROM `user` WHERE `id` = "' . $user . '" LIMIT 1 ')->row();
		...
		if ($type == 0 || $type == 2)
		{
			if (go\DB\query("SELECT COUNT(*) FROM `ban` WHERE `id_user` = '$user' AND (`time` > '$time' OR `navsegda` = '1')")->el() != 0)
			{
	...
	*/
	static function get_user($ID = 0, $photo = 1)
	{
		...
		*/
		...
		// Иконка пользователя
		if (go\DB\query("SELECT COUNT(*) FROM `ban` WHERE `id_user` = '$ID' AND (`time` > '" . time() . "' OR `navsegda` = '1')") != 0)
		{
