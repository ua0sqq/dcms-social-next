...
title();
if (!isset($_GET['go']) || ($db->query("SELECT COUNT(*) FROM `rekl` WHERE `id` = '".intval($_GET['go'])."'")==0 && !preg_match('#^https?://#',@base64_decode($_GET['go']))))
{
...
{
$rekl=$db->query("SELECT * FROM `rekl` WHERE `id` = '".intval($_GET['go'])."'")->row();
$db->query("UPDATE `rekl` SET `count` = '".($rekl['count']+1)."' WHERE `id` = '$rekl[id]'");
if (isset($_SESSION['adm_auth']))unset($_SESSION['adm_auth']);
