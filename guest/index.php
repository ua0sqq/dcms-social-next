...
/* Бан пользователя */ 
if (isset($user) && $db->query("SELECT COUNT(*) FROM `ban` WHERE `razdel` = 'guest' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0')")->el())
{
...
if (isset($_POST['msg']) && isset($user))
{
	...
	elseif ($db->query("SELECT COUNT(*) FROM `guest` WHERE `id_user` = '$user[id]' AND `msg` = '".my_esc($msg)."' LIMIT 1")->el())
	{
	...
	elseif(!isset($err))
	{
		...
		*/
		...
		if (isset($ank_reply['id']))
		{
			...
			$db->query("INSERT INTO `notification` (`avtor`, `id_user`, `type`, `time`) VALUES ('$user[id]', '$ank_reply[id]', 'guest', '$time')");
		}
    ...
    } elseif ($db->query("SELECT COUNT(*) FROM `guest` WHERE `id_user` = '0' AND `msg` = '".my_esc($msg)."' LIMIT 1")->el()) {
        $err = 'Ваше сообщение повторяет предыдущее';
    } elseif(!isset($err)) {
        $_SESSION['antiflood'] = $time;
        $db->query("INSERT INTO `guest` (id_user, time, msg) values('0', '$time', '".my_esc($msg)."')");
		$_SESSION['message'] = 'Сообщение успешно добавлено';
		header ("Location: index.php" . SID);
		exit;
    }
...
aut();
err();
$k_post = $db->query("SELECT COUNT(id) FROM `guest`")->el();
...
$q = $db->query("SELECT * FROM `guest` ORDER BY id DESC LIMIT $start, $set[p_str]");
while ($post = $q->row())
{
...
echo '<div class="foot">';
echo '<img src="/style/icons/str.gif" alt="*"> <a href="who.php">В гостевой (' . $db->query("SELECT COUNT(id) FROM `user` WHERE `date_last` > '".(time()-100)."' AND `url` like '/guest/%'")->el() . ' чел.)</a><br />';
echo '</div>';
