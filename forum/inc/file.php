...
if ($set['web'])
{
if ($db->query("SELECT COUNT(*) FROM `forum_files` WHERE `id_post` = '$post[id]'")>0)
{
?>
...
if (!isset($file['count']))$db->query("ALTER TABLE `forum_files` ADD `count` INT DEFAULT '0' NOT NULL");
echo "<td style='text-align:center;'>$file[count]</td>\n";
echo "<td style='text-align:center;'> ";
$k_vote=$db->query("SELECT COUNT(*) FROM `forum_files_rating` WHERE `id_file` = '$file[id]'");
$sum_vote=$db->query("SELECT SUM(`rating`) FROM `forum_files_rating` WHERE `id_file` = '$file[id]'");
if ($sum_vote==null)$sum_vote=0;
if (isset($user) && $user['balls']>=50 && $user['rating']>=0 && $db->query("SELECT COUNT(*) FROM `forum_files_rating` WHERE `id_user` = '$user[id]' AND `id_file` = '$file[id]'")==0)
