...
/* Бан пользователя */
if (isset($user) && $db->query("SELECT COUNT(*) FROM `ban` WHERE `razdel` = 'forum' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0' OR `navsegda` = '1')")!=0) {
...
$k_post = $db->query("SELECT COUNT(*) FROM `forum_t`  WHERE `id_user` = '$user[id]'")->el();
