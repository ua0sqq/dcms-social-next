...
if (isset($user) && isset($_GET['id']) && $db->query("SELECT COUNT(*) FROM `chat_rooms` WHERE `id` = '".intval($_GET['id'])."'")->el()
&& isset($_GET['msg']) && $db->query("SELECT COUNT(*) FROM `user` WHERE `id` = '".intval($_GET['msg'])."'")->el()) {
    $room=$db->query("SELECT * FROM `chat_rooms` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1")->row();
    $ank=$db->query("SELECT * FROM `user` WHERE `id` = '".intval($_GET['msg'])."' LIMIT 1")->row();
    if (isset($user)) {
...
$set['title']='Чат - '.$room['name'].' ('.$db->query("SELECT COUNT(*) FROM `chat_who` WHERE `room` = '$room[id]'")->el().')'; // заголовок страницы
...
if (isset($_GET['id']) && $db->query("SELECT COUNT(*) FROM `chat_rooms` WHERE `id` = '".intval($_GET['id'])."'")->el()) {
    $room=$db->query("SELECT * FROM `chat_rooms` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1")->row();
    if (isset($user)) {
        $db->query("INSERT INTO `chat_who` (`id_user`, `time`,  `room`) values('$user[id]', '$time', '$room[id]')");
    }
...
foreach ($q as $room) {
    ...
    echo "<a href='/chat/room/$room[id]/".rand(1000, 9999)."/'>$room[name] (".$db->query("SELECT COUNT(*) FROM `chat_who` WHERE `room` = '$room[id]'")->el().")</a> \n";
