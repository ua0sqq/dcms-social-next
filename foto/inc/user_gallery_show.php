...
/*---------------------------------------------------------*/
if (!isset($block_foto)) {
    $k_post = $db->query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id_gallery` = '$gallery[id]'")->el();
    ...
    $q = $db->query("SELECT * FROM `gallery_foto` WHERE `id_gallery` = '$gallery[id]' ORDER BY `id` DESC LIMIT $start, $set[p_str]");
    ...
    while ($post = $q->row()) {
        ...
        echo '<img src="/style/icons/uv.png"> (' . $db->query("SELECT COUNT(*) FROM `gallery_komm` WHERE `id_foto` = '$post[id]'")->el() . ')';
        echo '<img src="/style/icons/add_fav.gif"> ('.$db->query("SELECT COUNT(`id`)FROM `bookmarks` WHERE `id_object`='".$post['id']."' AND `type`='foto'")->el().')';
