...
if (isset($_POST['title']) && isset($_POST['msg']) && isset($_POST['link']) && isset($_POST['ok'])) {
    ...
    if (!isset($err)) {
        ...
        $news['id'] = $db->query("INSERT INTO `news` (`id_user`,`time`, `msg`, `title`, `main_time`, `link`) values('".$user['id']."','$time', '".$msg."', '$title', '$main_time', '$link')")->id();
        ...
        if (isset($_POST['mail'])) {
            ...
            while ($ank = $q->row()) {
                $db->query("INSERT INTO `mail_to_send` (`mail`, `them`, `msg`) values('$ank[ank_mail]', 'Новости', '".$msg."')");
            }
        }
