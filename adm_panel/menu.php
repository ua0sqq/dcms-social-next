...
include_once H . 'sys/inc/icons.php'; // Иконки главного меню
...
$pos=$db->query("SELECT MAX(`pos`) FROM `menu`")+1;
...
if (isset($_GET['id']) && isset($_GET['act']) && $db->query("SELECT COUNT(*) FROM `menu` WHERE `id` = '".intval($_GET['id'])."'")->el())
{
$menu=$db->query("SELECT * FROM `menu` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1")->row();
if ($_GET['act']=='up')
{
...
{
echo "   <tr>\n";
if (!isset($post['icon']))$db->query('ALTER TABLE `menu` ADD `icon` VARCHAR( 32 ) NULL DEFAULT NULL');
if (!isset($post['type']))$db->query("ALTER TABLE  `menu` ADD  `type` ENUM('link', 'razd') NOT NULL DEFAULT 'link' AFTER `id`");
echo "  <td class='p_t'>\n";
