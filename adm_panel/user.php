...
if (!$db->query("SELECT COUNT(*) FROM `user` WHERE `id` = '$ank[id]' LIMIT 1")->el()){header("Location: /index.php?".SID);exit;}
$ank=get_user($ank['id']);
...
{
if ($db->query("SELECT COUNT(*) FROM `user_group` WHERE `id` = '".intval($_POST['group_access'])."' AND `level` < '$user[level]'")->el())
{
