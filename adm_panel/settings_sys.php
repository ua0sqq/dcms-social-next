...
if (isset($_POST['save'])) {
    // ShaMan
    $temp_set['title'] = esc(filter_input(INPUT_POST, 'title', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
    // Тут конец моих     дум
    $temp_set['mail_backup'] = filter_input(INPUT_POST, 'mail_backup', FILTER_VALIDATE_EMAIL);
    $temp_set['p_str'] = intval($_POST['p_str']);
    $db->query("ALTER TABLE `user` CHANGE `set_p_str` `set_p_str` INT( 11 ) DEFAULT ?i",
               [$temp_set['p_str']]);
    ...
    $temp_set['meta_keywords'] = esc(filter_input(INPUT_POST, 'meta_keywords', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
    $temp_set['meta_description'] = esc(filter_input(INPUT_POST, 'meta_description', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
...
err();
aut();
echo "<form class=\"foot\" method=\"post\" action=\"?\">\n";
...
while ($themes=readdir($opendirthem)) {
    ...
    echo "<option value='$themes'" . ($temp_set['set_them'] == $themes ? " selected='selected'" : null) . ">" .
    trim(file_get_contents(H . 'style/themes/' . $themes . '/them.name')) . "</option>\n";
}
...
while ($themes=readdir($opendirthem)) {
    ...
    echo "<option value='$themes'" . ($temp_set['set_them2'] == $themes?" selected='selected'" : null) . ">" .
    trim(file_get_contents(H . 'style/themes/' . $themes . '/them.name')) . "</option>\n";
}
closedir($opendirthem);
echo "</select><br />\n";
