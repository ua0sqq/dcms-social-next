...
$mass[0]=$ank['id'];
$collisions=user_collision($mass, 1);
$chat_post=$db->query("SELECT COUNT(*) FROM `chat_post` WHERE `id_user` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $chat_post_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $chat_post_coll+=$db->query("SELECT COUNT(*) FROM `chat_post` WHERE `id_user` = '$collisions[$i]'")->el();
    }
    if ($chat_post_coll!=0) {
...
$k_them=$db->query("SELECT COUNT(*) FROM `forum_t` WHERE `id_user` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $k_them_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $k_them_coll+=$db->query("SELECT COUNT(*) FROM `forum_t` WHERE `id_user` = '$collisions[$i]'")->el();
    }
    if ($k_them_coll!=0) {
...
$k_p_forum=$db->query("SELECT COUNT(*) FROM `forum_p` WHERE `id_user` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $k_p_forum_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $k_p_forum_coll+=$db->query("SELECT COUNT(*) FROM `forum_p` WHERE `id_user` = '$collisions[$i]'")->el();
    }
    if ($k_p_forum_coll!=0) {
...
$zakl=$db->query("SELECT COUNT(*) FROM `forum_zakl` WHERE `id_user` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $zakl_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $zakl_coll+=$db->query("SELECT COUNT(*) FROM `forum_zakl` WHERE `id_user` = '$collisions[$i]'")->el();
    }
    if ($zakl_coll!=0) {
        $zakl="$zakl +$zakl_coll*";
    }
...
$guest=$db->query("SELECT COUNT(*) FROM `guest` WHERE `id_user` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $guest_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $guest_coll+=$db->query("SELECT COUNT(*) FROM `guest` WHERE `id_user` = '$collisions[$i]'")->el();
    }
    if ($guest_coll!=0) {
...
$konts=$db->query("SELECT COUNT(*) FROM `users_konts` WHERE `id_user` = '$ank[id]' OR `id_kont` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $konts_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $konts_coll+=$db->query("SELECT COUNT(*) FROM `users_konts` WHERE `id_user` = '$collisions[$i]' OR `id_kont` = '$collisions[$i]'")->el();
    }
    if ($konts_coll!=0) {
...
$mail=$db->query("SELECT COUNT(*) FROM `mail` WHERE `id_user` = '$ank[id]' OR `id_kont` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $mail_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $mail_coll+=$db->query("SELECT COUNT(*) FROM `mail` WHERE `id_user` = '$collisions[$i]' OR `id_kont` = '$collisions[$i]'")->el();
    }
    if ($mail_coll!=0) {
        $mail="$mail +$mail_coll*";
    }
...
if (count($collisions)>1 && isset($_GET['all'])) {
    $news_komm_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $news_komm_coll+=$db->query("SELECT COUNT(*) FROM `news_komm` WHERE `id_user` = '$collisions[$i]'")->el();
    }
    if ($news_komm_coll!=0) {
...
$user_voice=$db->query("SELECT COUNT(*) FROM `user_voice2` WHERE `id_user` = '$ank[id]' OR `id_kont` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $user_voice_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $user_voice_coll+=$db->query("SELECT COUNT(*) FROM `user_voice2` WHERE `id_user` = '$collisions[$i]' OR `id_kont` = '$collisions[$i]'")->el();
    }
...
$obmennik=$db->query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id_user` = '$ank[id]'")->el();
if (count($collisions)>1 && isset($_GET['all'])) {
    $obmennik_coll=0;
    for ($i=1;$i<count($collisions);$i++) {
        $obmennik_coll+=$db->query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id_user` = '$collisions[$i]'")->el();
    }
    if ($obmennik_coll!=0) {
...
if (count($collisions)>1 && isset($_GET['all'])) {
    ...
    for ($i=1;$i<count($collisions);$i++) {
        $ank_coll=$db->query("SELECT * FROM `user` WHERE `id` = '$collisions[$i]'")->row();
        echo "$ank_coll[nick]";
