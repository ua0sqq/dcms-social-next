...
$db->query('TRUNCATE `chat_shutnik`');
$k_add=0;
...
$db->query("INSERT INTO `chat_shutnik` (`anek`) VALUES ('".my_esc($shut)."')");
$k_add++;
}
...
err();
aut();
echo "Всего шуток в базе: ".$db->query("SELECT COUNT(*) FROM `chat_shutnik`")->el()."<br />\n";
