...
if (isset($_SERVER['HTTP_ACCEPT_ENCODING']) && preg_match('#deflate#', $_SERVER['HTTP_ACCEPT_ENCODING'])) {
...
if ($Content_Encoding['deflate']) {
    ...
    ob_start();
} // если нет сжатия, то просто буферизация данных
session_name('SESS');
session_start();
$sess=session_id();
...
<title>Ошибка $err</title>\n";
    ...
    if ($err=='400') {
        echo "Обнаруженная ошибка в запросе\n";
    } elseif ($err=='401') {
        echo "Нет прав для выдачи документа\n";
    } elseif ($err=='402') {
        echo "Не реализованный код запроса\n";
    } elseif ($err=='403') {
        ...
        echo "Сервер получил недопустимые ответы другого сервера\n";
    } else {
