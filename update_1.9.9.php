...
if(!isset($_GET['step'])){
 ...
 $db->query("alter table `forum_t` add `vote_close` enum('0','1') default '0'");
 $db->query("alter table `forum_t` add `vote` varchar(32) default NULL");
 $db->query("CREATE TABLE `votes_forum` (
...
$db->query("CREATE TABLE `votes_user` (
  `id` int(11) NOT NULL auto_increment,
  `them` int(11) NOT NULL,
  `var` varchar(32) NOT NULL,
 ...
 $db->query("CREATE TABLE `stena_komm` (
 ...
 $db->query("alter table `notes` add `share` enum('0','1') default '0'");
 $db->query("alter table `notes` add `share_type` varchar(12) default 'notes'");
 $db->query("alter table `notes` add `share_id` int(11) default null");
 $db->query("alter table `notes` add `share_id_user` int(11) default null");
 }
 ...
 $db->query("alter table `forum_r` add `opis` varchar(256) default NULL");
 }
 }
