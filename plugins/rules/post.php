...
$k_post=$db->query("SELECT COUNT(*) FROM `rules`");
...
if ($user['level'] > 2){
	...
	if (isset($_POST['msg']) && isset($user))
	{
		...
		elseif ($db->query("SELECT COUNT(*) FROM `rules_p` WHERE `id_news` = '".intval($_GET['id'])."' AND `id_user` = '$user[id]' AND `msg` = '".my_esc($msg)."' LIMIT 1")!=0){$err='Ваше сообщение повторяет предыдущее';}
		elseif(!isset($err)){
			$pos=$db->query("SELECT MAX(`pos`) FROM `rules_p` WHERE `id_news` = '".intval($_GET['id'])."'")+1;
	...
	if (isset($_GET['ids']))$menu=$db->query("SELECT * FROM `rules_p` WHERE `id` = '".intval($_GET['ids'])."' LIMIT 1")->row();
...
$k_post=$db->query("SELECT COUNT(*) FROM `rules_p` WHERE `id_news` = '".intval($_GET['id'])."'");
