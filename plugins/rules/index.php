...
if (isset($user) && $user['level'] > 2){
	if (isset($_GET['del']) && is_numeric($_GET['del']) && $db->query("SELECT COUNT(*) FROM `rules` WHERE `id` = '".intval($_GET['del'])."' LIMIT 1",$db)==1)
	{
...
if (isset($_GET['id']) && isset($_GET['act']) && $db->query("SELECT COUNT(*) FROM `rules` WHERE `id` = '".intval($_GET['id'])."'"))
{
