...
===============================
*/	
$db->query("UPDATE `notification` SET `read` = '1' WHERE `type` = 'adm_komm' AND `id_user` = '$user[id]'");
...
elseif ($db->query("SELECT COUNT(*) FROM `adm_chat` WHERE `id_user` = '$user[id]' AND `msg` = '".my_esc($msg)."' LIMIT 1")->el()){$err='Ваше сообщение повторяет предыдущее';}
elseif(!isset($err)){
		/*
		==========================
...
$num=0;
$q=$db->query("SELECT * FROM `adm_chat` ORDER BY id DESC LIMIT $start, $set[p_str]");
while ($post = $q->row())
{
/*-----------зебра-----------*/
if ($num==0)
