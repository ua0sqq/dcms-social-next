...
if (isset($_GET['id']) && $db->query("SELECT COUNT(*) FROM `spamus` WHERE `id` = '".intval($_GET['id'])."'")->el())
{
$post=$db->query("SELECT * FROM `spamus` WHERE `id` = '".intval($_GET['id'])."' LIMIT 1")->row();
...
$db->query("INSERT INTO `mail` (`id_user`, `id_kont`, `msg`, `time`) values('0', '$ank[id]', '".my_esc($msg)."', '$time')");
$db->query("DELETE FROM `spamus` WHERE `id` = '$post[id]'");
}
}
