...
$k_post=$db->query("SELECT COUNT(*) FROM `spamus` $types")->el();
...
if ($post['razdel'] == 'files_komm'){  // Файлы юзеров
	$file_id = $db->query("SELECT * FROM `obmennik_files` WHERE `id` = '$post[id_object]' LIMIT 1")->row();
	$dir = $db->query("SELECT * FROM `user_files` WHERE `id` = '$file_id[my_dir]' LIMIT 1")->row();
...
if ($post['razdel'] == 'loads_komm'){  // Загрузки
	$komm=$db->query("SELECT * FROM `loads_komm` WHERE `id` = '$post[id_object]' LIMIT 1")->row();
	$file=$db->query("SELECT * FROM `loads_list` WHERE `name` = '$komm[file]' LIMIT 1")->row();
