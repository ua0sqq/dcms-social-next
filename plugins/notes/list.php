...
/* Бан пользователя */
if (isset($user) && $db->query("SELECT COUNT(*) FROM `ban` WHERE `razdel` = 'notes' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0')")->el()) {
...
if (isset($user)) {
    $count = $db->query("SELECT COUNT(*) FROM `notes_count` WHERE `id_user` = '".$user['id']."' AND `id_notes` = '".$notes['id']."' LIMIT 1")->el();
}
 // Закладки
$markinfo = $db->query("SELECT COUNT(*) FROM `bookmarks` WHERE `id_object` = '".$notes['id']."' AND `type`='notes'")->el();
if (isset($user)) {
...
if (isset($_GET['spam'])  &&  isset($user)) {
    ...
    if (!$db->query("SELECT COUNT(*) FROM `spamus` WHERE `id_user` = '$user[id]' AND `id_spam` = '$spamer[id]' AND `razdel` = 'notes_komm' AND `spam` = '".$mess['msg']."'")->el()) {
        if (isset($_POST['msg'])) {
    ...
    aut();
    err();
    if (!$db->query("SELECT COUNT(*) FROM `spamus` WHERE `id_user` = '$user[id]' AND `id_spam` = '$spamer[id]' AND `razdel` = 'notes_komm'")->el()) {
...
*/
// Запись просмотра
if (isset($user) && !$db->query("SELECT COUNT(*) FROM `notes_count` WHERE `id_user` = '".$user['id']."' AND `id_notes` = '".$notes['id']."' LIMIT 1")->el()) {
...
if (isset($_POST['msg']) && isset($user)) {
    ...
    } elseif ($db->query("SELECT COUNT(*) FROM `notes_komm` WHERE `id_notes` = '".intval($_GET['id'])."' AND `id_user` = '$user[id]' AND `msg` = '".my_esc($msg)."' LIMIT 1")->el()) {
        ...
        */
        if (isset($user) && $respons==true) {
            ...
            if ($notifiacation['komm'] == 1 && $ank_reply['id'] != $user['id']) {
                $db->query("INSERT INTO `notification` (`avtor`, `id_user`, `id_object`, `type`, `time`) VALUES ('$user[id]', '$ank_reply[id]', '$notes[id]', 'notes_komm', '$time')");
            }
        }
        /*
        ====================================
...
{
//---------друзьям автора--------------//
if (!$db->query("SELECT COUNT(*) FROM `discussions` WHERE `id_user` = '$a[id]' AND `type` = 'notes' AND `id_sim` = '$notes[id]' LIMIT 1")->el()) {
...
//-------------------------------------//
        ...
        //-------------отправляем автору------------//
        if (!$db->query("SELECT COUNT(*) FROM `discussions` WHERE `id_user` = '$notes[id_user]' AND `type` = 'notes' AND `id_sim` = '$notes[id]' LIMIT 1")->el()) {
...
if (isset($user)) {
    $frend = $db->query("SELECT COUNT(*) FROM `frends` WHERE (`user` = '$user[id]' AND `frend` = '$avtor[id]') OR (`user` = '$avtor[id]' AND `frend` = '$user[id]') LIMIT 1")->el();
}
title();
...
if (isset($user)) {
    if (isset($_GET['like']) && $_GET['like'] == 1) {
        if (!$db->query("SELECT COUNT(*) FROM `notes_like` WHERE `id_user` = '".$user['id']."' AND `id_notes` = '".$notes['id']."' LIMIT 1")->el()) {
    ...
    if (isset($_GET['fav']) && $_GET['fav']==1) {
        if (!$db->query("SELECT COUNT(*) FROM `bookmarks` WHERE `id_user` = '".$user['id']."' AND `id_object` = '".$notes['id']."' AND `type`='notes' LIMIT 1")->el()) {
...
$k_1=$db->query("SELECT COUNT(*) FROM `notes` WHERE `id` > '$notes[id]'")->el()+1;
$k_2=$db->query("SELECT COUNT(*) FROM `notes`")->el();
echo ' ('.$k_1.' из '.$k_2.') ';
echo '<span class="page">' . ($listr['id'] ? '<a href="list.php?id=' . $listr['id'] . '">След. &raquo;</a>' : ' След. &raquo;') . '</span>';
echo '</div>';
/*----------------------plugins---------------*/
echo "<div class='main2'>";
$share=$db->query("SELECT COUNT(*)FROM `notes` WHERE `share_id`='".$notes['id']."' AND `share_type`='notes'")->el();
if (!$db->query("SELECT COUNT(*)FROM `notes` WHERE `id_user`='".$user['id']."' AND `share_type`='notes' AND `share_id`='".$notes['id']."' LIMIT 1")->el() && isset($user) && $user['id']!=$notes['id_user']) {
...
$l1=$db->query("SELECT COUNT(*) FROM `notes_like` WHERE `like` = '0' AND `id_notes` = '".$notes['id']."' LIMIT 1")->el();
$l2=$db->query("SELECT COUNT(*) FROM `notes_like` WHERE `like` = '1' AND `id_notes` = '".$notes['id']."' LIMIT 1")->el();
...
===================================
*/
$k_post=$db->query("SELECT COUNT(*) FROM `notes_komm` WHERE `id_notes` = '".intval($_GET['id'])."'")->el();
$k_page=k_page($k_post, $set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];
echo '<div class="foot">';
echo "<b>Комментарии</b>: (".$db->query("SELECT COUNT(`id`)FROM `notes_komm` WHERE `id_notes`='".$notes['id']."'")->el().")\n";
echo '</div>';
if ($k_post==0) {
    echo '<div class="mess">';
    echo "Нет сообщений\n";
...
echo "</table>\n"; if ($k_page>1) {
    str("list.php?id=".intval($_GET['id']).'&amp;', $k_page, $page);
} // Вывод страниц
