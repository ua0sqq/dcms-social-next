...
/* Бан пользователя */ 
if ($db->query("SELECT COUNT(*) FROM `ban` WHERE `razdel` = 'notes' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0' OR `navsegda` = '1')")!=0)
{
...
$k_post=$db->query("SELECT COUNT(*) FROM `notes` WHERE `id_user` = '$ank[id]' ");
