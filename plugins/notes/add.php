...
if (isset($_POST['title']) && isset($_POST['msg'])) {
    ...
    if (!isset($err)) {
        ...
        if (!$id_dir = filter_input(INPUT_POST, 'id_dir', FILTER_VALIDATE_INT)) {
            $err = 'Не выбрана категория!';
        }
        if ($id_dir && !$db->query('SELECT COUNT( * ) FROM `notes_dir` WHERE `id`=?i',
                          [$id_dir])->el()) {
            $err = 'Категория не найдена!';
        }
        if ($id_dir && $db->query('SELECT COUNT( * ) FROM `notes` WHERE `name`=? AND `id_dir`=?i AND `id_user`=?i',
                          [$title, $id_dir, $user['id']])->el()) {
            $err = 'Дневник с таким названием уже есть в этой категории.';
        }
...
$id_dir = filter_input(INPUT_GET, 'id_dir', FILTER_VALIDATE_INT);
...
$q=$db->query("SELECT * FROM `notes_dir` ORDER BY `id` DESC")->assoc();

echo "Категория:<br />\n<select name='id_dir'>\n";

if (empty($q)) {
    $data = ['msg' => 'Дневники на отвлеченные темы', 'name' => 'Без категории'];
    $id_dir = $db->query('INSERT INTO `notes_dir` SET ?set',
                        [$data])->id();
    echo "<option value='$id_dir' selected='selected'><b>Без категории</b></option>\n";
}
...
echo "</select><br />\n";
echo "<div class='main'>Могут смотреть:<br /><input name='private' type='radio' value='0' checked=\"checked\"/>Все ";
echo "<input name='private' type='radio'  value='1' />Друзья ";
echo "<input name='private' type='radio'  value='2' />Только я</div>";
 
echo "<div class='main'>Могут комментировать:<br /><input name='private_komm' type='radio' value='0' checked=\"checked\"/>Все ";
