...
/* Бан пользователя */ 
if (isset($user) && $db->query("SELECT COUNT(*) FROM `ban` WHERE `razdel` = 'notes' AND `id_user` = '$user[id]' AND (`time` > '$time' OR `view` = '0' OR `navsegda` = '1')")!=0)
{
...
$new=null; }
$k_post=$db->query("SELECT COUNT(*) FROM `notes` WHERE `private`='0'");
$k_page=k_page($k_post,$set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];
$q=$db->query("SELECT * FROM `notes` $order LIMIT $start, $set[p_str]");
...
notes_sh($post['id']);
echo "<br/><img src='/style/icons/uv.png'> <font color=#666>(".$db->query("SELECT COUNT(`id`)FROM `notes_komm` WHERE `id_notes`='$post[id]'").") &bull;";
echo " <a href='fav.php?id=".$post['id']."'><img src='/style/icons/add_fav.gif'> (".$db->query("SELECT COUNT(`id`)FROM `bookmarks` WHERE `id_object`='".$post['id']."' AND `type`='notes'").")</a> &bull; ";
echo " <img src='/style/icons/action_share_color.gif'> (".$db->query("SELECT COUNT(`id`)FROM `notes` WHERE `share_id`='".$post['id']."' AND `share_type`='notes'").") </font>";
echo "  </div>\n";
}
echo "</table>\n";
