...
if ($type == 'obmen')
{
	$dir = $db->query("SELECT * FROM `user_files` WHERE `id` = '" . $post['id_file'] . "' LIMIT 1")->row();
	...
	echo '<div class="nav2">';
	$files = $db->query("SELECT * FROM `obmennik_files` WHERE `my_dir` = '$dir[id]' ORDER BY `id` DESC LIMIT $kol");
