...
if ($type  ==  'album')
{
	$gallery = $db->query("SELECT * FROM `gallery` WHERE `id` = '".$post['id_file']."' LIMIT 1")->row();
	...
	if ($gallery['id'])
	{
		...
		echo '</div>';
		echo '<div class="nav2">';
		$as = $db->query("SELECT * FROM `gallery_foto` WHERE `id_gallery` = '$gallery[id]' ORDER BY `id` DESC LIMIT $kol");
