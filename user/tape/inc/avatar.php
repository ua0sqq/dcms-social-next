...
if ($type == 'avatar')
{
	$foto = $db->query("SELECT * FROM `gallery_foto` WHERE `id` = '".$post['id_file']."' LIMIT 1")->row();
	$avatar = $db->query("SELECT * FROM `gallery_foto` WHERE `id` = '".$post['avatar']."' LIMIT 1")->row();
	$gallery = $db->query("SELECT * FROM `gallery` WHERE `id` = '".$foto['id_gallery']."' LIMIT 1")->row();
	$gallery2 = $db->query("SELECT * FROM `gallery` WHERE `id` = '".$avatar['id_gallery']."' LIMIT 1")->row();
	echo '<div class="nav1">';
