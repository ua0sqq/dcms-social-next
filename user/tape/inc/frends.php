...
if ($type == 'frends')
{
	...
	if ($frend['id'])
	{
		...
		if ($db->query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id_user` = '$frend[id]'")>0)
		{
			echo 'Последние добавленные фото ' . user::nick($frend['id'], 0) . '<br />';
			
			$g = $db->query("SELECT * FROM `gallery_foto` WHERE `id_user` = '$frend[id]' ORDER BY `id` DESC LIMIT 4");
			
			while ($xx = $g->row())
			{
				$gallery = $db->query("SELECT * FROM `gallery` WHERE `id` = '" . $xx['id_gallery'] . "' LIMIT 1")->row();
