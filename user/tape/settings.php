...
title();
$lentaSet = $db->query("SELECT * FROM `tape_set` WHERE `id_user` = '".$user['id']."' LIMIT 1")->row();
if (isset($_POST['save'])){
 // Лента фото
if (isset($_POST['lenta_foto']) && ($_POST['lenta_foto']==0 || $_POST['lenta_foto']==1))
{
$db->query("UPDATE `tape_set` SET `lenta_foto` = '".intval($_POST['lenta_foto'])."' WHERE `id_user` = '$user[id]'");
}
 // Лента файлов
if (isset($_POST['lenta_files']) && ($_POST['lenta_files']==0 || $_POST['lenta_files']==1))
{
$db->query("UPDATE `tape_set` SET `lenta_files` = '".intval($_POST['lenta_files'])."' WHERE `id_user` = '$user[id]'");
}
 // Лента смены аватара
if (isset($_POST['lenta_avatar']) && ($_POST['lenta_avatar']==0 || $_POST['lenta_avatar']==1))
{
$db->query("UPDATE `tape_set` SET `lenta_avatar` = '".intval($_POST['lenta_avatar'])."' WHERE `id_user` = '$user[id]'");
}
 // Лента новых друзей
if (isset($_POST['lenta_frends']) && ($_POST['lenta_frends']==0 || $_POST['lenta_frends']==1))
{
$db->query("UPDATE `tape_set` SET `lenta_frends` = '".intval($_POST['lenta_frends'])."' WHERE `id_user` = '$user[id]'");
}
 // Лента статусов
if (isset($_POST['lenta_status']) && ($_POST['lenta_status']==0 || $_POST['lenta_status']==1))
{
$db->query("UPDATE `tape_set` SET `lenta_status` = '".intval($_POST['lenta_status'])."' WHERE `id_user` = '$user[id]'");
}
 // Лента оценок статуса
if (isset($_POST['lenta_status_like']) && ($_POST['lenta_status_like']==0 || $_POST['lenta_status_like']==1))
{
$db->query("UPDATE `tape_set` SET `lenta_status_like` = '".intval($_POST['lenta_status_like'])."' WHERE `id_user` = '$user[id]'");
}
 // Лента дневников
if (isset($_POST['lenta_notes']) && ($_POST['lenta_notes']==0 || $_POST['lenta_notes']==1))
{
$db->query("UPDATE `tape_set` SET `lenta_notes` = '".intval($_POST['lenta_notes'])."' WHERE `id_user` = '$user[id]'");
}
 // Лента форума
if (isset($_POST['lenta_forum']) && ($_POST['lenta_forum']==0 || $_POST['lenta_forum']==1))
{
$db->query("UPDATE `tape_set` SET `lenta_forum` = '".intval($_POST['lenta_forum'])."' WHERE `id_user` = '$user[id]'");
}
