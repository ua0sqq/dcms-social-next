...
$k_post=$db->query("SELECT COUNT(*) FROM `user_music` WHERE `id_user` = '$ank[id]'");
...
while ($post = $q->row())
{
$mp3 = $db->query("SELECT * FROM `obmennik_files` WHERE `id` = '$post[id_file]' LIMIT 1")->row();
$dir = $db->query("SELECT * FROM `obmennik_dir` WHERE `id` = '$mp3[id_dir]' LIMIT 1")->row();
$ras = $mp3['ras'];
/*-----------зебра-----------*/
if ($num==0)
{echo "  <div class='nav1'>\n";
