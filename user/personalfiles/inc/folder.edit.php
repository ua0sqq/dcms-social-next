...
if (isset($_GET['edit_folder'])) {
    ...
    if ($db->query(
			'SELECT COUNT( * ) FROM `user_files` WHERE `id_user`=?i AND `name`=? AND `id_dires`=? AND `id`<>?i',
                   [$user['id'], $name, $folder['id_dires'], $folder['id']])->el()) {
        $err[] = 'Такая папка уже существует!';
    }
        if (!isset($err)) {
            $db->query(
                'UPDATE `user_files` SET `name`=?,  `pass`=?, `msg`=? WHERE `id`=?i',
                       [$name, $pass, $msg, $folder['id']]);
