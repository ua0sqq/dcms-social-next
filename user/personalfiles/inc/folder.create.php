...
if (isset($_POST['name']) && isset($user)) {
    ...
    $osn = $dir['osn']+1;
    if ($dir['osn'] == 6) {
        $err[] = 'Нельзя создавать папки в более чем двух поддиректорях';
    }
    if (strlen2($msg)>256) {
    ...
    if ($db->query('SELECT COUNT( * ) FROM `user_files` WHERE `id_user`=?i AND `name`=? AND `id_dires`=?',
                   [$user['id'], $name, $id_dires])->el()) {
        $err[] = 'Такая папка уже существует!';
    }
    if (!isset($err)) {
        $db->query('INSERT INTO `user_files` (`id_user`, `name`, `msg`,  `time`, `id_dir`, `osn`, `id_dires`, `pass`) VALUES(?i, ?, ?,  ?i, ?i, ?i, ?, ?n)',
                   [$user['id'], $name, $msg, $time, $dir['id'], $osn, $id_dires, $pass]);
