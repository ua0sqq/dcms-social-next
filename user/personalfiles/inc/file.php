...
=======================================
*/
$file_id=$db->query("SELECT * FROM `obmennik_files` WHERE `id`='".intval($_GET['id_file'])."' LIMIT 1")->row();
...
$dir_id=$db->query("SELECT * FROM `obmennik_dir` WHERE `id` = '$file_id[id_dir]' LIMIT 1")->row();
$ras=$file_id['ras'];
...
if (!$db->query("SELECT COUNT(*) FROM `spamus` WHERE `id_user` = '$user[id]' AND `id_spam` = '$spamer[id]' AND `razdel` = 'files_komm' AND `spam` = '".$mess['msg']."'")->el())
{
if (isset($_POST['msg']))
{
...
if (!isset($err))
{
$db->query("INSERT INTO `spamus` (`id_object`, `id_user`, `msg`, `id_spam`, `time`, `types`, `razdel`, `spam`) values('$file_id[id]', '$user[id]', '$msg', '$spamer[id]', '$time', '$types', 'files_komm', '".my_esc($mess['msg'])."')");
...
aut();
err();
if (!$db->query("SELECT COUNT(*) FROM `spamus` WHERE `id_user` = '$user[id]' AND `id_spam` = '$spamer[id]' AND `razdel` = 'files_komm'")->el())
{
...
if (isset($user))
{
$db->query("UPDATE `discussions` SET `count` = '0' WHERE `id_user` = '$user[id]' AND `type` = 'obmen' AND `id_sim` = '$file_id[id]' LIMIT 1");
}
/*---------------------------------------------------------*/
/*------------------------Мне нравится------------------------*/
...
include_once '../../sys/inc/thead.php';
title();
if ((user_access('obmen_komm_del') || $ank['id'] == $user['id']) && isset($_GET['del_post']) && $db->query("SELECT COUNT(*) FROM `obmennik_komm` WHERE `id` = '".intval($_GET['del_post'])."' AND `id_file` = '$file_id[id]'")->el())
{
...
if (isset($user))
$db->query("UPDATE `notification` SET `read` = '1' WHERE `type` = 'files_komm' AND `id_user` = '$user[id]' AND `id_object` = '$file_id[id]'");
...
elseif ($db->query("SELECT COUNT(*) FROM `obmennik_komm` WHERE `id_file` = '$file_id[id]' AND `id_user` = '$user[id]' AND `msg` = '".my_esc($msg)."' LIMIT 1")->el()){$err='Ваше сообщение повторяет предыдущее';}
elseif(!isset($err)){
...
====================================
*/
$q = $db->query("SELECT * FROM `frends` WHERE `user` = '".$file_id['id_user']."' AND `i` = '1'");
while ($f = $q->row())
{
$a=get_user($f['frend']);
...
/*----------------------листинг-------------------*/
$listr = $db->query("SELECT * FROM `obmennik_files` WHERE `my_dir` = '$dir[id]' AND `id` < '$file_id[id]' ORDER BY `id` DESC LIMIT 1")->row();
$list = $db->query("SELECT * FROM `obmennik_files` WHERE `my_dir` = '$dir[id]' AND `id` > '$file_id[id]' ORDER BY `id`  ASC LIMIT 1")->row();
...
$k_1=$db->query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id` > '$file_id[id]' AND `my_dir` = '$dir[id]'")->el()+1;
$k_2=$db->query("SELECT COUNT(*) FROM `obmennik_files` WHERE `my_dir` = '$dir[id]'")->el();
echo ' ('.$k_1.' из '.$k_2.') ';
...
/*----------------------alex-borisi---------------*/
echo '<div class="main">';
if (isset($user) && $ank['id'] != $user['id'] && !$db->query("SELECT COUNT(*) FROM `like_object` WHERE `id_object` = '$file_id[id]' AND `type` = 'obmen' AND `id_user` = '$user[id]'")->el())
{
...
'.$db->query("SELECT COUNT(*) FROM `like_object` WHERE `id_object` = '$file_id[id]' AND `type` = 'obmen' AND `like` = '1'")->el().'] ';
echo '[<img src="/style/icons/dlike.gif" alt="*"> 
'.$db->query("SELECT COUNT(*) FROM `like_object` WHERE `id_object` = '$file_id[id]' AND `type` = 'obmen' AND `like` = '0'")->el().']';
}
echo '</div>';
echo '<div class="main">';
if ($file_id['ras']=='jar')
