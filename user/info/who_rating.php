...
if (isset($_POST['rating']) && isset($user) && isset($_POST['msg']) && $user['id']!=$ank['id'] && $user['rating']>=2 && $db->query("SELECT SUM(`rating`) FROM `user_voice2` WHERE `id_kont` = '$user[id]'")>=0)
{
	...
	elseif ($db->query("SELECT COUNT(*) FROM `user_voice2` WHERE `id_user` = '$user[id]' AND `msg` = '".my_esc($msg)."' LIMIT 1")!=0){$err='Ваш отзыв повторяется';}
	if (!isset($err))
	{
...
$k_post=$db->query("SELECT COUNT(*) FROM `user_voice2` WHERE `id_kont` = '".$ank['id']."'");
