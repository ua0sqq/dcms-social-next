...
//$not_user=$db->query("SELECT COUNT(*) FROM `user` WHERE `id` = '$ank[id]' LIMIT 1")->el();
...
while ($post3 = $q->row()) {
    ...
    echo '<a href="/mail.php?id='.$ank3['id'].'"><img src="/style/icons/pochta.gif" alt="*" /> Сообщение</a> ';
    echo "</div>";
}
/*---------------------the end--------------------------*/
?>
...
echo '<a class="top_nav" href="/user/frends/?id='.$ank['id'].'">Друзья ('.$k_fr.'</b>/';
$i =0;
while ($k_fr = $res->row()) {
    if ($db->query("SELECT COUNT(*) FROM `user` WHERE `id` = '$k_fr[frend]' && `date_last` > '".(time()-800)."'")->el()) {
        $i++;
    }
}
echo $i;
if ($k_f>0 && $ank['id']==$user['id']) {
    echo " +".$k_f."";
}
echo "</a>";
/*--------------------------------------------------------------*/
...
echo "(" . $db->query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id_user` = '$ank[id]'")->el() . ")</a>";
/*--------------------------------------------------------------*/
/*-------------------------Личные файлы---------------------------*/
...
$db->query("SELECT COUNT(*) FROM `obmennik_files` WHERE `id_user` = '$ank[id]'")->el() . ")";
echo "</a>";
/*----------------------------------------------------------------*/
...
if ($status['id'] || $ank['id'] == $user['id']) {
    ...
    if ($status['id']) {
        echo output_text($status['msg']) . ' <font style="font-size:10px; color:gray;">' . vremja($status['time']) . '</font>';
    }
    echo "</div>";
    if ($status['id']) {
...
===============================
*/
$sql = $db->query("SELECT * FROM `gallery_foto` WHERE `id_user` = '$ank[id]' ORDER BY `id` DESC LIMIT 8");
$coll = $db->query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id_user` = '$ank[id]' ORDER BY `id` DESC")->el();
if ($coll>0) {
    echo "<div class='slim_header'>";
    echo "<img src='/style/icons/pht2.png' alt='*' /> ";
    echo "<a href='/foto/$ank[id]/'><b>Фотографии</b></a> ";
    echo " <span class='mm_counter'>" . $db->query("SELECT COUNT(*) FROM `gallery_foto` WHERE `id_user` = '$ank[id]'")->el() . "</span>";
    echo "</div>";
    echo "<div class='nav3'>";
...
if ($k_p > 0) {
    ...
    $q = $db->query("SELECT id,id_gift,status FROM `gifts_user` WHERE `id_user` = '$ank[id]' AND `status` = '1' ORDER BY `id` DESC LIMIT 7");
    echo '<div class="nav2">';
    while ($post = $q->row()) {
