...
echo "</div>\n";
$k_post=$db->query("SELECT COUNT(*) FROM `gallery_rating` WHERE `avtor` = '$ank[id]'")->el();
$k_page=k_page($k_post,$set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];
$q=$db->query("SELECT * FROM `gallery_rating` WHERE `avtor` = '$ank[id]' ORDER BY `time` DESC LIMIT $start, $set[p_str]");
if ($k_post==0)
{
echo "  <div class='mess'>\n";
echo "Нет оценок\n";
...
$foto=$db->query("SELECT * FROM `gallery_foto` WHERE `id` = $post[id_foto]")->row();
if ($foto['id'] && $ank2['id'])
{
$gallery=$db->query("SELECT * FROM `gallery` WHERE `id` = $foto[id_gallery]")->row();
//-----------зебра-----------//
if ($num==0)
{echo "  <div class='nav1'>\n";
$num=1;
...
$db->query("DELETE FROM `gallery_rating` WHERE `avtor` = '$post[avtor]' AND `id_foto` = '$post[id_foto]'");
}
}
$db->query("UPDATE `gallery_rating` SET `read`='0' WHERE `avtor` = '$user[id]' AND `read`='1'");
