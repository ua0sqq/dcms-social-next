...
title();
$notSet = $db->query("SELECT * FROM `notification_set` WHERE `id_user` = '".$user['id']."' LIMIT 1")->row();
if (isset($_POST['save'])){
 // Комментарии
if (isset($_POST['komm']) && ($_POST['komm']==0 || $_POST['komm']==1))
{
$db->query("UPDATE `notification_set` SET `komm` = '".intval($_POST['komm'])."' WHERE `id_user` = '$user[id]'");
}
