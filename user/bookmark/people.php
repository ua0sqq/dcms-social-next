...
{
$db->query("DELETE FROM `bookmarks` WHERE `id` = '" . intval($_GET['delete']) . "' AND `id_user` = '$user[id]' AND `type`='people' LIMIT 1");
...
echo '</div>';
$k_post=$db->query("SELECT COUNT(*) FROM `bookmarks` WHERE `id_user` = '$ank[id]' AND `type`='people' ")->el();
...
$q=$db->query("SELECT * FROM `bookmarks` WHERE `id_user` = '$ank[id]' AND `type`='people' ORDER BY id DESC LIMIT $start, $set[p_str]");
while ($post = $q->row())
{
$ank_p = $db->query("SELECT * FROM `user` WHERE `id` = $post[id_object] LIMIT 1")->row();
/*-----------зебра-----------*/ 
if ($num==0){
	echo '<div class="nav1">';
	$num=1;
