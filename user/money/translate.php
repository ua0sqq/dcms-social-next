...
if (!$err)
{
$db->query("UPDATE `user` SET `money` = '" . ($ank['money'] + $money) . "' WHERE `id` ='$ank[id]';");
$db->query("UPDATE `user` SET `money` = '" . ($user['money'] - $money) . "' WHERE `id` ='$user[id]';");
$msg = "Пользователь [b]".$user['nick']."[/b] перевeл вам средства в колличестве [b] $money [/b] $sMonet[0]! [br]НЕ ЗАБУДЬТЕ СКАЗАТЬ СПАСИБО!";
$db->query("INSERT INTO `mail` (`id_user`, `id_kont`, `msg`, `time`) values('0', '$ank[id]', '$msg', '$time')");
