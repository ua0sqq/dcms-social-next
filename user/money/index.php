...
echo "</div>\n";
echo '<div class="nav1">';
$c = $db->query("SELECT COUNT(*) FROM `liders` WHERE `id_user` = '$user[id]' AND `time` > '$time'")->el();
echo '&rarr; <a href="liders.php">Лидер сайта</a> ' . ($c == 0 ? '<span class="off">[отключена]</span> ' : '<span class="on">[включена]</span>');
echo '</div>';
echo '<div class="nav2">';
$c2 = $db->query("SELECT COUNT(*) FROM `user_set` WHERE `id_user` = '$user[id]' AND `ocenka` > '$time'")->el();
