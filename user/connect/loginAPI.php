...
if (isset($_POST['token']) && !isset($user) && $users['network'] && $users['identity'] && $_POST['loginAPI'] == true) {
    if (!$db->query("SELECT COUNT(*) FROM `user` WHERE `type_reg` = '" . $users['network'] . "' AND `identity` = '" . $users['identity'] . "'")->el()) {
        /*
        ================================
        Имя к id и пол
        ================================
        ...
        */
        ...
        ================================
        */
        if ($db->query("SELECT COUNT(*) FROM `user` WHERE `nick` = '" . $identity . "'")->el()) {
 ...
 values('1', '$identity', '" . shif($pass) . "', '$time', '$time', '" . $pol . "', '" . $users['city'] . "', '" . $users['first_name'] . "', '" . $users['identity'] . "', '" . $users['network'] . "')", $db);
        $user=$db->query("SELECT * FROM `user` WHERE `nick` = '". $identity ."' AND `pass` = '". shif($pass) ."' LIMIT 1")->row();// отправка сообщения
        ...
        */
        ...
        $gallery=$db->query("SELECT * FROM `gallery` WHERE `id_user` = '$user[id]'  LIMIT 1")->row();
        if ($photo) { // Наличие фото
            if ($imgc=@imagecreatefromstring(file_get_contents($photo))) {
                ...
                $img_y=imagesy($imgc);
                if (!isset($err)) {
                    $id_foto=$db->query("INSERT INTO `gallery_foto` (`id_gallery`, `name`, `ras`, `type`, `id_user`,`avatar`) values ('$gallery[id]', '$name', 'jpg', 'image/jpeg', '$user[id]','1')")->id();
...
imagejpeg($screen, H."sys/gallery/640/$id_foto.jpg", 90);
        ...
        $user=$db->query("SELECT * FROM `user` WHERE `type_reg` = '" . $users['network'] . "' AND `identity` = '" . $users['identity'] . "' LIMIT 1")->row();
