...
only_reg();
	...
	// Подарок
	$post = $db->query("SELECT id,status,coment,id_gift,id_ank,id_user,time FROM `gifts_user` WHERE `id` = '" . intval($_GET['id']) . "' LIMIT 1")->row();
	
	// Если записи нет кидаем на главную
	if (!$post['id']) { header("Location: /index.php?"); } 
	
	 // Сам Подарок 
	$gift = $db->query("SELECT id,name FROM `gift_list` WHERE `id` = '" . $post['id_gift'] . "' LIMIT 1")->row();
