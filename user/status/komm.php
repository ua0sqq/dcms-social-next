...
*/
	...
	$frend=$db->query("SELECT COUNT(*) FROM `frends` WHERE (`user` = '$user[id]' AND `frend` = '$anketa[id]') OR (`user` = '$anketa[id]' AND `frend` = '$user[id]') LIMIT 1");
	$frend_new=$db->query("SELECT COUNT(*) FROM `frends_new` WHERE (`user` = '$user[id]' AND `to` = '$anketa[id]') OR (`user` = '$anketa[id]' AND `to` = '$user[id]') LIMIT 1");
...
{
$mess = $db->query("SELECT * FROM `status_komm` WHERE `id` = '".intval($_GET['spam'])."' limit 1")->row();
$spamer = get_user($mess['id_user']);
if ($db->query("SELECT COUNT(*) FROM `spamus` WHERE `id_user` = '$user[id]' AND `id_spam` = '$spamer[id]' AND `razdel` = 'status_komm' AND `spam` = '".$mess['msg']."'")==0)
{
if (isset($_POST['msg']))
{
...
if (!isset($err))
{
$db->query("INSERT INTO `spamus` (`id_object`, `id_user`, `msg`, `id_spam`, `time`, `types`, `razdel`, `spam`) values('$status[id]', '$user[id]', '$msg', '$spamer[id]', '$time', '$types', 'status_komm', '".my_esc($mess['msg'])."')");
...
aut();
err();
if ($db->query("SELECT COUNT(*) FROM `spamus` WHERE `id_user` = '$user[id]' AND `id_spam` = '$spamer[id]' AND `razdel` = 'status_komm'")==0)
{
...
if (isset($user))
{
$db->query("UPDATE `discussions` SET `count` = '0' WHERE `id_user` = '$user[id]' AND `type` = 'status' AND `id_sim` = '$status[id]' LIMIT 1");
}
/*---------------------------------------------------------*/
if (isset($user))
$db->query("UPDATE `notification` SET `read` = '1' WHERE `type` = 'status_komm' AND `id_user` = '$user[id]' AND `id_object` = '$status[id]'");
...
elseif ($db->query("SELECT COUNT(*) FROM `status_komm` WHERE `id_status` = '".intval($_GET['id'])."' AND `id_user` = '$user[id]' AND `msg` = '".my_esc($msg)."' LIMIT 1")!=0){$err='Ваше сообщение повторяет предыдущее';}
elseif(!isset($err)){
		/*
		==========================
...
====================================
*/
$q = $db->query("SELECT * FROM `frends` WHERE `user` = '".$status['id_user']."' AND `i` = '1'");
while ($f = $q->row())
{
$a=get_user($f['frend']);
...
{
	// друзьям автора
	if ($db->query("SELECT COUNT(*) FROM `discussions` WHERE `id_user` = '$a[id]' AND `type` = 'status' AND `id_sim` = '$status[id]' LIMIT 1")==0)
	{
	if ($status['id_user']!=$a['id'] || $a['id'] != $user['id'])
	$db->query("INSERT INTO `discussions` (`id_user`, `avtor`, `type`, `time`, `id_sim`, `count`) values('$a[id]', '$status[id_user]', 'status', '$time', '$status[id]', '1')"); 
	}
	else
	{
	$disc = $db->query("SELECT * FROM `discussions` WHERE `id_user` = '$status[id_user]' AND `type` = 'status' AND `id_sim` = '$status[id]' LIMIT 1")->row();
	if ($status['id_user']!=$a['id'] || $a['id']!= $user['id'])
	$db->query("UPDATE `discussions` SET `count` = '".($disc['count']+1)."', `time` = '$time' WHERE `id_user` = '$a[id]' AND `type` = 'status' AND `id_sim` = '$status[id]' LIMIT 1");
	}
}
}
// отправляем автору
if ($db->query("SELECT COUNT(*) FROM `discussions` WHERE `id_user` = '$status[id_user]' AND `type` = 'status' AND `id_sim` = '$status[id]' LIMIT 1")==0)
{
if ($status['id_user'] != $user['id'])
$db->query("INSERT INTO `discussions` (`id_user`, `avtor`, `type`, `time`, `id_sim`, `count`) values('$status[id_user]', '$status[id_user]', 'status', '$time', '$status[id]', '1')"); 
}
else
{
$disc = $db->query("SELECT * FROM `discussions` WHERE `id_user` = '$status[id_user]' AND `type` = 'status' AND `id_sim` = '$status[id]' LIMIT 1")->row();
if ($status['id_user'] != $user['id'])
$db->query("UPDATE `discussions` SET `count` = '".($disc['count']+1)."', `time` = '$time' WHERE `id_user` = '$status[id_user]' AND `type` = 'status' AND `id_sim` = '$status[id]' LIMIT 1");
}
$db->query("INSERT INTO `status_komm` (`id_user`, `time`, `msg`, `id_status`) values('$user[id]', '$time', '".my_esc($msg)."', '".intval($_GET['id'])."')");
...
echo "Комментарии:\n";
echo "</div>";
$k_post=$db->query("SELECT COUNT(*) FROM `status_komm` WHERE `id_status` = '".intval($_GET['id'])."'");
$k_page=k_page($k_post,$set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];
$q=$db->query("SELECT * FROM `status_komm` WHERE `id_status` = '".intval($_GET['id'])."' ORDER BY `id` DESC LIMIT $start, $set[p_str]");
...
$postBan = $db->query("SELECT COUNT(*) FROM `ban` WHERE (`razdel` = 'all') AND `post` = '1' AND `id_user` = '$ank[id]' AND (`time` > '$time' OR `navsegda` = '1')");
